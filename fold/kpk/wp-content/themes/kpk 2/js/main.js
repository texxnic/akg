...
jQuery(document).ready(function($) {
    ...
    // formValidation
    var phoneImput = $("input[name=phone]");
    var privacy = $("#privacyCheck");
    var phoneImput = $("#contactPhoneID");
    var inputed = $(this).val();

    privacy.click(function(event) {
        $(".checkBoxText").css('color', 'white');
    });
    phoneImput.keypress(function(e) {
        inputed = $(this).val();
        // var inputedLenth = $(this).val().length;

        if (inputed.search("_") == 15 || inputed.search("_") == -1) {
            phoneImput.css('border-bottom', '1px solid white');
        };
    });
    $(".contactSubmit").click(function(event) {
        ...
        if (inputedSearch) {
            phoneImput.css('border-bottom', '1px solid red');
            ...
            if (privacy.is(":checked")) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $(".contactDiv").css('border-color', '#52c2ee');
                        $(".contactHidden").hide('4000', function() {});
                        $(".formButtonText").html("Спасибо! Ваша заявка отправлена, скоро мы вам перезвоним.");


                    },
                    error: function(data) {
                        $(".contactDiv").css('border-color', 'red');
                        $(".formButtonText").html("Извините, что-то пошло не так. Попробуйте еще раз, или позвоните пожалуйста по телефону 8(800)234-44-44 (звонок бесплатный)");
    ...
    if ($("#PageID").hasClass("index")) {
        $(".menuLink1").addClass('menuLinkActive');
    } else if ($("#PageID").hasClass("kpkVklad")) {
        $(".menuLink2").addClass('menuLinkActive');
    } else if ($("#PageID").hasClass("vkladCap")) {
        $(".menuLink3").addClass('menuLinkActive');
    } else if ($("#PageID").hasClass("curVklad")) {
        $(".menuLink4").addClass('menuLinkActive');
    } else if ($("#PageID").hasClass("about")) {
        $(".menuLink1").addClass('menuLinkActive');
    } else if ($("#PageID").hasClass("contacts")) {
        $(".menuLink5").addClass('menuLinkActive');
    };
    ...
    function calculateCur(e) {
        ...
        // проценты без капитализации валюта
        switch (simpleRange) {
            case 180:
                simpleRangeYear = 0.5;
        ...
        function numberDivider(a0,a1,a2){
            for (var i = 0; i <= 2; i++) {
                let varname = "a" + i; 
