...
foreach ($recent_posts2 as $recent) {
	echo '<div class="second-news post-image has_overlay" style="background-image: url(\' ' . get_the_post_thumbnail_url($recent["ID"]) . '\');"> <a href="' . get_permalink($recent["ID"]) . '">' . '<h4>' . $recent["post_title"] . '</h4>' . '   </div>';
}
wp_reset_query();
?>
                        </div>
