<div class="header">
    <a href="../"><img src="assets/img/logoW.svg" alt="" class="header__logo"></a>
    <img src="assets/img/menuHamburg.svg" class="header__menuButton" alt="">
    <div class="header__menu">
        <div class="header__topMenu">
            <div class="header__languageSelector">
                <a href="" class="selectedText">RU</a>
                <a href="">EN</a>
            </div>
            <a href="../contacts.php">
                <button class="leaveReply leaveReply__opacity">
                    Оставить заявку
                </button></a>
                <a class="header__phone phoneBig" href="tel:88002344444">8 800 234-44-44</a>
            </div>
            <div class="header__botMenu">
                <a href="/" class="menuLink selected">
                    <div class="menuLink__div">
                        <div class="unskewed">Услуги</div>
                    </div>
                </a>
                <a href="/about.php" class="menuLink">
                    <div class="menuLink__div">
                        <div class="unskewed">О нас</div>
                    </div>
                </a>
                <a href="../prices.php" class="menuLink">
                    <div class="menuLink__div">
                        <div class="unskewed">Цены</div>
                    </div>
                </a>
                <a href="../faq.php" class="menuLink">
                    <div class="menuLink__div">
                        <div class="unskewed">FAQ</div>
                    </div>
                </a>
                <!-- <a href="../testom" class="menuLink">
                    <div class="menuLink__div">
                        <div class="unskewed">Отзывы</div>
                    </div>
                </a> -->
                <a href="../contacts.php" class="menuLink">
                    <div class="menuLink__div">
                        <div class="unskewed">Контакты</div>
                    </div>
                </a>
            </div>
        </div>
    </div>
    <div class="mobileMenu DN">
        <a href="/" class="mobileMenuLink selected"><h1>Услуги</h1></a>
        <a href="/about.php" class="mobileMenuLink"><h1>О нас</h1></a>
        <a href="../prices.php" class="mobileMenuLink"><h1>Цены</h1></a>
        <a href="../faq.php" class="mobileMenuLink"><h1>FAQ</h1></a>
        <!-- <a href="../testom" class="mobileMenuLink"><h1>Отзывы</h1></a> -->
        <a href="../contacts.php" class="mobileMenuLink"><h1>Контакты</h1></a>
    </div>